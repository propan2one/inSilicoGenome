...
def write_fasta_genome(output, sequence, description='', line_width=80):
    """
    Writes name/sequence to file in FASTA format, the header will be the name of
      the variable. In addition, a description string can be added.
    ...
    sequence : A sequence of DNA between 1 and 100 000 000bp
      A string generated by random_dnasequence
    description : Description of the sequence (empty by default). This is useful
      to facilitate the creation of an abundance distribution table. 
    line_width : Line width when outputting FASTA format (0 for no wrap)
      (default 80)
    ...
    n = line_width
    split_sequence = [sequence[i:i+line_width] for i in range(0, len(sequence), n)]
    with open(output, 'w') as outfile:
            outfile.write(f">{header}\n")
            for item in split_sequence:
                outfile.write(f"{item}\n")
    print(f"insilicogenome::write_fasta_genome::[info]: sequence {output} have"
          f" been succefully create")
